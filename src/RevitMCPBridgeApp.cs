...
namespace RevitMCPBridge
{
    public class RevitMCPBridgeApp : IExternalApplication
    {
        ...
        private void CreateServerPanel(UIControlledApplication application)
        {
            ...
            // ── Saved Scripts ─────────────────────────────────────────────
            var scriptsPanel = application.CreateRibbonPanel(_tabName, "Automation");
            var runScriptButtonData = new PushButtonData("RunSavedScript", "Scripts", asm,
                "RevitMCPBridge.Commands.RunSavedScriptCommand")
            {
                ToolTip = "Run a saved script — zero tokens, instant execution",
                LongDescription = "Pick a C# script from your firm's automation library and run it directly in Revit.\n\n" +
                    "Scripts are generated by Banana Chat and saved to your library. " +
                    "Running them here uses the Roslyn compiler locally — no Claude call, no token cost.",
            };
            var runScriptButton = scriptsPanel.AddItem(runScriptButtonData) as PushButton;
            runScriptButton.LargeImage = CreateButtonIcon("scripts", 32);
            runScriptButton.Image      = CreateButtonIcon("scripts", 16);
        ...
        private static void ApplyButtonKeyTips()
        {
            ...
            var keyTips = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                ...
                { "RunSavedScript",    "L" },  // Scripts     — Alt→BM→L
                { "BimMonkeyPlatform", "W" },
                { "StartMCPServer",    "1" },
                { "StopMCPServer",     "2" },
                { "MCPServerStatus",   "3" },
        ...
        private void DrawScriptsIcon(DrawingContext dc, int size)
        {
            // Three code lines (left) + right-pointing play triangle (right) — "run a script"
            double s       = size / 32.0;
            var fill       = new SolidColorBrush(Colors.White);
            var dark       = new SolidColorBrush(Color.FromRgb(60, 60, 60));
            var outlinePen = new Pen(dark, Math.Max(1, 1.5 * s));
            var linePen    = new Pen(dark, Math.Max(1, 1.8 * s))
                { StartLineCap = PenLineCap.Round, EndLineCap = PenLineCap.Round };

            // Play triangle — right-pointing, right half of canvas
            double tx = 17 * s, ty = 6 * s, tw = 11 * s, th = 20 * s;
            var tri = new StreamGeometry();
            using (var ctx = tri.Open())
            {
                ctx.BeginFigure(new Point(tx, ty),             true, true);
                ctx.LineTo(new Point(tx + tw, ty + th * 0.5), true, false);
                ctx.LineTo(new Point(tx, ty + th),             true, false);
            }
            dc.DrawGeometry(fill, outlinePen, tri);

            // Three code lines — left half of canvas
            double lx1 = 4 * s, lx2 = 13 * s;
            dc.DrawLine(linePen, new Point(lx1, 9 * s),  new Point(lx2, 9 * s));
            dc.DrawLine(linePen, new Point(lx1, 16 * s), new Point(lx2, 16 * s));
            dc.DrawLine(linePen, new Point(lx1, 23 * s), new Point(lx1 + (lx2 - lx1) * 0.65, 23 * s));
        }
