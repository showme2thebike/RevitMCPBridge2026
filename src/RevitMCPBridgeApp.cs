...
namespace RevitMCPBridge
{
    public class RevitMCPBridgeApp : IExternalApplication
    {
        ...
        private void CreateServerPanel(UIControlledApplication application)
        {
            ...
            // ── Saved Scripts ─────────────────────────────────────────────
            var scriptsPanel = application.CreateRibbonPanel(_tabName, "Scripts");
            var runScriptButtonData = new PushButtonData("RunSavedScript", "Run\nScript", asm,
                "RevitMCPBridge.Commands.RunSavedScriptCommand")
            {
                ToolTip = "Run a saved Banana Chat script — zero tokens, instant execution",
                LongDescription = "Pick a C# script from your firm's library and run it directly in Revit.\n\n" +
                    "Scripts are generated by Banana Chat and saved from the web app. " +
